  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "register") {
        ...
        toast.success("Usuario registrado. Ahora inicia sesión."); // <-- toast
        onClose();
      } else {
        await login(form.email, form.password);
        toast.success("Bienvenido!"); // <-- toast login
        onClose();
      }
    } catch (err: any) {
      console.error(err);
      toast.error(err.response?.data?.message || "Error inesperado."); // <-- toast error
    } finally {
      setLoading(false);
    }
  };
